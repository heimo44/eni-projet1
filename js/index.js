...
.catch((error) => {
    console.error('Une erreur est survenue lors de la recuperation du fichier JSON', error)
})
...
function openModalOnDetail(apprenants){
    for (let i = 1; i <= apprenants.length; i++){
        document.getElementById(`detail-${i}`).style.cssText = 'text-decoration: underline; cursor: pointer'
    }
}